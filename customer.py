...
# === 老闆通知冷卻＋去重（防「找真人」灌爆）===
STAFF_NOTIFY_COOLDOWN = 600  # 同一用戶 10 分鐘內最多通知老闆一次
...
# === 記憶體狀態清理（避免 in-memory dict 無限成長）===
_IDLE_TTL = 24 * 3600       # 用戶閒置超過 24h 即清除其對話/計數
_CLEANUP_INTERVAL = 3600    # 最多每小時掃一次
_last_seen: dict[str, float] = {}
_last_cleanup = 0.0


def _touch(user_id: str):
    _last_seen[user_id] = time.time()


def _maybe_cleanup():
    """週期性淘汰閒置用戶與過期日期的狀態，避免記憶體緩慢洩漏。"""
    ...
    stale = [u for u, t in _last_seen.items() if now - t > _IDLE_TTL]
    for u in stale:
    ...
    # 非今日的額度／提示記錄一律清掉
    for u in [k for k, v in list(_daily_counts.items()) if v.get("date") != today]:
        _daily_counts.pop(u, None)
    for u in [k for k, v in list(_off_hours_notified.items()) if v != today]:
        _off_hours_notified.pop(u, None)
...
- 備料需求收集（務必查證，資訊不齊或不合理一律追問，不可貿然記錄）：
  收集順序：品名+數量 → 工地地址 → 進退場日期 → 付款方式 → 聯絡電話。
  【地址查證】必須是司機找得到、可配送的完整地址（縣市＋區＋路名＋門牌號）。若客人只給「台中」「工地附近」「某某路」這種模糊資訊，要追問：「為了讓司機準確送達，方便給完整地址嗎？例如○○市○○區○○路○號。」地址不完整前不可記錄。
  【電話查證】必須是合理的台灣號碼（手機 09 開頭共 10 碼，或市話含區碼）。若位數不對、明顯亂填（如 0000、1234、一串重複數字），要客氣再確認：「這支號碼好像不太對，方便再確認一次嗎？葉經理要直接打給您。」號碼不合理前不可記錄。
  缺電話時問「方便留個電話讓葉經理直接聯絡您嗎？這樣比較快。」此時不可說「已記錄」。
  【記錄前複誦確認】資訊到齊後，先把「品項數量、地址、日期、電話」整理給客人確認（例：「跟您確認一下：乳膠漆白色 5 桶、送台中市北屯區○○路 12 號、0912-345-678，對嗎？」）。複誦這步不要說「已記錄」。
  客人確認無誤後，才說「我已經幫您記錄下來：…」並列完整清單。只要地址或電話有疑慮，就不要記錄，繼續客氣查證。
...
- 客人需求明確「且地址完整、電話合理」時才記錄；資訊模糊或可疑仍要先查證，不可省略
